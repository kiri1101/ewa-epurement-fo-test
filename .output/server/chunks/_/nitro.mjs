import process from 'node:process';globalThis._importMeta_=globalThis._importMeta_||{url:"file:///_entry.js",env:process.env};import { relations, sql, eq, or, like, lt, and } from 'drizzle-orm';
...
const _inlineRuntimeConfig = {
  "app": {
    "baseURL": "/",
    "buildId": "89f8dc1d-2324-45f4-9b06-218022ee3f26",
  ...
  "public": {
    "baseUrl": "https://ewa-epurement-fo-test-qnyzjywy.on-forge.com",
    ...
    "api": {
      ...
      "createManage": "/api/profile/manager",
      "managerList": "/api/profile/collaboratorsList",
      "collaborator": {
        "search": "/api/profile/collaboratorSearch"
      },
      "beneficiary": {
      ...
      "transfer": {
        ...
        "search": "/api/demand/search",
        "types": "/api/demand/types",
        "setting": "/api/demand/setting",
        "create": "/api/demand/create",
        "otp": {
          "request": "/api/demand/requestOtp",
          "validate": "/api/demand/validateOtp"
        }
      },
  ...
  en: [
    {
      ...
      load: () => import('./en.mjs' /* webpackChunkName: "locale_en_46json_d6c92f75" */),
      cache: true
    }
  ],
  fr: [
    {
      ...
      load: () => import('./fr.mjs' /* webpackChunkName: "locale_fr_46json_2c1437e4" */),
      cache: true
    }
  ]
};
...
const bankAcc = sqliteTable("bank_accounts", {
  id: int().primaryKey({ autoIncrement: true }),
  uuid: text().notNull().unique(),
  userId: int("user_id").references(() => users.id, {
    onDelete: "cascade",
...
const bankAccRelations = relations(bankAcc, ({ one, many }) => ({
...
const rolesRelations = relations(roles, ({ one }) => ({
...
const collaborators = sqliteTable("collaborators", {
  ...
  accId: int("acc_id").references(() => bankAcc.id, {
    onDelete: "cascade",
    onUpdate: "cascade"
  }).notNull(),
  username: text().notNull(),
...
const beneficiaryFiles = sqliteTable("beneficiary_files", {
  id: int().primaryKey({ autoIncrement: true }),
  uuid: text().notNull().unique(),
...
const beneficiaries = sqliteTable("beneficiaries", {
  id: int().primaryKey({ autoIncrement: true }),
  uuid: text().notNull().unique(),
  userId: int("user_id").references(() => users.id, {
    onDelete: "cascade",
...
const beneficiariesRelations = relations(beneficiaries, ({ one }) => ({
...
const transferTypes = sqliteTable("transfer_types", {
  id: int().primaryKey({ autoIncrement: true }),
  uuid: text().notNull().unique(),
  code: text().notNull(),
  name: text().notNull()
});
const transferTypesRelations = relations(transferTypes, ({ many }) => ({
  attachments: many(transferAttachments)
}));
const transferAttachments = sqliteTable("transfer_attachments", {
  id: int().primaryKey({ autoIncrement: true }),
  uuid: text().notNull().unique(),
  typeId: int("type_id").references(() => transferTypes.id, {
    onDelete: "cascade",
    onUpdate: "cascade"
  }).notNull(),
  name: text().notNull(),
...
const transferAttachmentsRelations = relations(
  transferAttachments,
  ({ one }) => ({
...
const schema = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.defineProperty({
  __proto__: null,
  bankAcc: bankAcc,
  bankAccRelations: bankAccRelations,
  beneficiaries: beneficiaries,
  beneficiariesRelations: beneficiariesRelations,
  beneficiaryFiles: beneficiaryFiles,
  collaborators: collaborators,
  collaboratorsRelations: collaboratorsRelations,
  countries: countries,
  ...
  transferAttachmentsRelations: transferAttachmentsRelations,
  transferTypes: transferTypes,
  transferTypesRelations: transferTypesRelations,
  transfers: transfers,
...
const client = createClient({ url: config$1.private.dbPath });
...
const getAccounts = async (event) => {
  ...
  let response = null;
  if (user) {
    let authUser = await db.select({ id: users.id }).from(users).where(eq(users.uuid, user == null ? void 0 : user.id));
    let result = await db.select({ id: bankAcc.uuid, name: bankAcc.bank, value: bankAcc.accRef }).from(bankAcc).where(eq(bankAcc.userId, authUser[0].id));
    response = result.length > 0 ? result : null;
  }
  return response;
};

const saveBeneficiary = async (event, response) => {
  const auth = authCookie(event);
  const authUser = auth.getUserSnapShot();
  let output;
  if (authUser) {
    const user = await db.select({ userId: users.id }).from(users).where(eq(users.uuid, authUser.id));
    ...
    if (response.length > authUserBeneficiary.length) {
      output = await db.transaction(async (tx) => {
        await tx.delete(beneficiaries).where(eq(beneficiaries.userId, user[0].userId));
...
const createBeneficiary = async (tx, response, user) => {
  const beneficiaryMap = response.map(
    (data) => {
      var _a;
      const uuid = crypto.randomUUID();
      return {
        uuid,
        userId: (_a = user[0]) == null ? void 0 : _a.userId,
        code: data.benefSlug,
        type: data.benefType,
...
const saveCollaborators = async (event, response) => {
  const auth = authCookie(event);
  const authUser = auth.getUserSnapShot();
  let output;
  if (authUser) {
    ...
    if (response.length > authUserCol.length) {
      output = await db.transaction(async (tx) => {
        await tx.delete(collaborators).where(eq(collaborators.userId, user[0].userId));
...
const createCollaborators = async (tx, response, user) => {
  const collaboratorMap = await Promise.all(
    response.map(async (res) => {
      var _a, _b;
      const uuid = crypto.randomUUID();
      const bankAccount = await tx.select({ accountId: bankAcc.id }).from(bankAcc).where(eq(bankAcc.iBan, res.bankinfo.accountNumber));
      return {
        uuid,
        ...
        userId: (_a = user[0]) == null ? void 0 : _a.userId,
        accId: (_b = bankAccount[0]) == null ? void 0 : _b.accountId,
...
const searchCollaborators = async (index) => {
...
async function loadLocale(lang) {
  let output, locale;
  try {
    locale = lang === "en" ? await import('./en2.mjs') : await import('./fr2.mjs');
    output = locale.default.validation;
  } catch (error) {
    const fallback = await import('./en2.mjs');
    output = fallback.default.validation;
  }
  return output;
}

const saveCountries = async (event, countryList, now) => {
  return await db.transaction(async (tx) => {
    await tx.delete(countries).where(lt(countries.createdAt, now));
    const insertValues = countryList.map((country) => {
...
const saveFiles = async (event, fileList, now) => {
  return await db.transaction(async (tx) => {
    const latestNow = moment().format("YYYY-MM-DD");
    await tx.delete(beneficiaryFiles).where(lt(beneficiaryFiles.createdAt, now));
    const companyFiles = fileList.MORALE.map((file) => {
      const uuid = crypto.randomUUID();
      return {
        uuid,
        fileId: file.docId,
    ...
    const individualFiles = fileList.PHYSIQUE.map(
      (file) => {
        const uuid = crypto.randomUUID();
        return {
          uuid,
    ...
    const insertValues = companyFiles.concat(individualFiles);
...
const saveTransfer = async (event, response) => {
  const auth = authCookie(event);
  const authUser = auth.getUserSnapShot();
  let output = null;
  if (authUser) {
...
const createTransfer = async (tx, response, user) => {
  const transferMap = response.map(
    (data) => {
      var _a;
      const uuid = crypto.randomUUID();
      return {
...
const searchTransfers = async (index) => {
  return await db.select({
    code: transfers.code,
    beneficiaryName: transfers.beneficiaryName,
    amount: transfers.amount,
...
const saveTransferTypes = async (response) => {
  ...
  if (response.length > types.length) {
    ...
    await db.transaction(async (tx) => {
      for (const type of response) {
        const storedType = await tx.insert(transferTypes).values({
          uuid: crypto.randomUUID(),
          code: type.demandeType,
          name: type.demandeTypeName
        }).returning({ insertId: transferTypes.id });
        const attachmentMap = type.attachmentList.map((attachment) => {
          return {
            uuid: crypto.randomUUID(),
...
const updateTransferOtp = async (code, otpCode, otpLiveTime) => {
  await db.update(transfers).set({
    otpCode,
    otpLiveTime
  }).where(eq(transfers.code, code));
};
const isOtpValidate = async (code, otpCode) => {
...
const getOtp = async (event, otpCode) => {
  const auth = authCookie(event);
  const user = auth.getUserSnapShot();
  let response = null;
  if (user) {
    let result = await db.select({ otp: users.otp }).from(users).where(and(eq(users.uuid, user == null ? void 0 : user.id), eq(users.otp, otpCode)));
...
const saveUser = async (data) => {
  ...
  return await db.transaction(async (tx) => {
    try {
      ...
      if (authUser) {
        if ((authUser == null ? void 0 : authUser.sessions.length) > 0) {
        ...
        const user = await tx.insert(users).values(userData).returning({ insertId: users.uuid, id: users.id });
        sessionId = await createSession(tx, {
          uuid: uuid2,
          userId: user[0].id,
          createdAt: now,
...
const createSession = async (tx, sessionData) => {
  const newSession = await tx.insert(sessions).values(sessionData).returning({ insertId: sessions.uuid });
  return newSession[0].insertId;
};
const createRole = async (tx, roleData, user) => {
  const roleMap = roleData.map((role) => {
    const uuid = crypto.randomUUID();
    return {
      uuid,
      userId: user.id,
...
const createBankAcc = async (tx, bankData, user) => {
  const bankAccMap = bankData.map((acc) => {
    const uuid = crypto.randomUUID();
    return {
      uuid,
      userId: user.id,
...
const deleteUser = async (sessionId) => {
  try {
    ...
    if (currentSessionWithUser) {
      if ((currentSessionWithUser == null ? void 0 : currentSessionWithUser.user.sessions.length) > 1) {
        await db.delete(sessions).where(eq(sessions.uuid, sessionId));
      } else {
        await db.delete(users).where(eq(users.id, currentSessionWithUser.user.id));
      }
    }
  } catch (error) {
  }
...
const _aEZLAq_FcGVdK7lvTKpjZjCLUBfblRxMGP3XZ_fPCc = defineNitroPlugin$1(async (nitro) => {
  ...
  nitro.hooks.hook("render:before", async ({ event }) => {
    ...
    if (!url.pathname.includes("/_i18n/xo7DqSez") && !isExistingNuxtRoute(path)) {
      return;
    }
...
const assets = {
  ...
  "/_nuxt/18c95hkh.js": {
    ...
    "etag": "\"134e-iUpRlVxwC3znzupfMkW+vmpJuQE\"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 4942,
    "path": "../public/_nuxt/18c95hkh.js"
  },
  "/_nuxt/3ELfzwXa.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"6800-JSe2GnuK/+mg8RLusteekQcaexY\"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 26624,
    "path": "../public/_nuxt/3ELfzwXa.js"
  },
  "/_nuxt/3wY-Ku_v.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"462-JCR3vQL2zXolp5gQ4FONpm6kt2o\"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 1122,
    "path": "../public/_nuxt/3wY-Ku_v.js"
  },
  "/_nuxt/45dbutmm.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"1bce-hbAnOCDPy+nIOyEuVBidqrS8Ytg\"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 7118,
    "path": "../public/_nuxt/45dbutmm.js"
  },
  "/_nuxt/4IQlD9I2.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"aca-WjNEQI6lzUSu2U7U3DA9tih7hpc\"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 2762,
    "path": "../public/_nuxt/4IQlD9I2.js"
  },
  "/_nuxt/4xnLMnx7.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"796-XCNkyoZw+7Di6WSUE6zXzj5UhFM\"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 1942,
    "path": "../public/_nuxt/4xnLMnx7.js"
  },
  "/_nuxt/9Hz1IUOn.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"e7f-L7pcNp69xerzNUkpmKqMoWrpXrU\"",
    "mtime": "2026-01-19T22:39:05.073Z",
    "size": 3711,
    "path": "../public/_nuxt/9Hz1IUOn.js"
  },
  "/_nuxt/accounts.Cd3Y_NUC.css": {
    "type": "text/css; charset=utf-8",
    "etag": "\"6b-XePepwMAi2WUScsqOnNh1CJ4cfE\"",
    "mtime": "2026-01-19T22:39:05.073Z",
    "size": 107,
    "path": "../public/_nuxt/accounts.Cd3Y_NUC.css"
  },
  "/_nuxt/ATZ6OQ3d.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"128-n7lAwBPVx0kAEMs/nvareffNYhk\"",
    "mtime": "2026-01-19T22:39:05.073Z",
    "size": 296,
    "path": "../public/_nuxt/ATZ6OQ3d.js"
  },
  "/_nuxt/auth.qVjUTK-1.css": {
    "type": "text/css; charset=utf-8",
    "etag": "\"2c-68h4FviFJMtwJlfL9SowlFCKA2Q\"",
    "mtime": "2026-01-19T22:39:05.073Z",
    "size": 44,
    "path": "../public/_nuxt/auth.qVjUTK-1.css"
  },
  "/_nuxt/B1qJ81tr.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"1d68-76sk8D9sh742sDOeFz0lmCxAb4w\"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 7528,
    "path": "../public/_nuxt/B1qJ81tr.js"
  },
  "/_nuxt/B3aIy2kQ.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"1154-BWGTUx6mHIDJY6pYzb0OEz1pmbo\"",
    "mtime": "2026-01-19T22:39:05.074Z",
    "size": 4436,
    "path": "../public/_nuxt/B3aIy2kQ.js"
  },
  "/_nuxt/B3ikrqk1.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"6a9-ic3teMxKRV1ZbWtGt+cNu7lW5w0\"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 1705,
    "path": "../public/_nuxt/B3ikrqk1.js"
  },
  "/_nuxt/B4fKgFjA.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"1400-296KNs+G0op8rSomTmXckL17Bmw\"",
    "mtime": "2026-01-19T22:39:05.074Z",
    "size": 5120,
    "path": "../public/_nuxt/B4fKgFjA.js"
  },
  "/_nuxt/B5DrMY_E.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"736-qgdLoU/bjBVbHTSS2mcjLG9s4aY\"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 1846,
    "path": "../public/_nuxt/B5DrMY_E.js"
  },
  "/_nuxt/B7YrqMVw.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"25e-yhI9ldu8Odcf9P8SUfmrf3Oa7vg\"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 606,
    "path": "../public/_nuxt/B7YrqMVw.js"
  },
  "/_nuxt/B8CzfCTi.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"100b-mKMFObYAbMPxz5idlKF835l96qY\"",
    "mtime": "2026-01-19T22:39:05.073Z",
    "size": 4107,
    "path": "../public/_nuxt/B8CzfCTi.js"
  },
  "/_nuxt/BaC6Kzth.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"5d3a-z0/VBipeLNDZ7sAWKb1Wqlo68zE\"",
    "mtime": "2026-01-19T22:39:05.074Z",
    "size": 23866,
    "path": "../public/_nuxt/BaC6Kzth.js"
  },
  "/_nuxt/BaGcUojI.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"ad2-8zhdtsvtaDwvzqrHazfX/AlgjZQ\"",
    "mtime": "2026-01-19T22:39:05.077Z",
    "size": 2770,
    "path": "../public/_nuxt/BaGcUojI.js"
  },
  "/_nuxt/BBpuFvPC.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"168f-Igu8I/OiR+3tlWtrK16jInb64X8\"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 5775,
    "path": "../public/_nuxt/BBpuFvPC.js"
  },
  "/_nuxt/BBrqcZ_w.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"32ec1-bsoREpZp6xxnXVHkHxq+ah+neRM\"",
    "mtime": "2026-01-19T22:39:05.074Z",
    "size": 208577,
    "path": "../public/_nuxt/BBrqcZ_w.js"
  },
  "/_nuxt/BDYGyTok.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"227d-X0bNGnmlz9aq38kKHjK9MIp9G6g\"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 8829,
    "path": "../public/_nuxt/BDYGyTok.js"
  },
  "/_nuxt/beneficiary.C3ykcJcR.css": {
    "type": "text/css; charset=utf-8",
    "etag": "\"6a-Qc5q3JWB+aCDFpikzf5VfGbFuzg\"",
    "mtime": "2026-01-19T22:39:05.073Z",
    "size":
"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 43414,
    "path": "../public/_nuxt/Cigx3Dqg.js"
  },
  "/_nuxt/CIKxzYOE.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"40c5-tIt3FhLYHcaVCgMEvWmSIzKO/zw\"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 16581,
    "path": "../public/_nuxt/CIKxzYOE.js"
  },
  "/_nuxt/CIUYLZ3s.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"121-TbuSiZ1o7IoKIjnfsDTL52s7nys\"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 289,
    "path": "../public/_nuxt/CIUYLZ3s.js"
  },
  "/_nuxt/CiwSZdk0.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"4f0-3VvfIWsBmI3MGyW0Vns6cWkF2IQ\"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 1264,
    "path": "../public/_nuxt/CiwSZdk0.js"
  },
  "/_nuxt/CjX9tCQr.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"67e8-f9gEDnVJF5S6cG0O41rjmaP5d5g\"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 26600,
    "path": "../public/_nuxt/CjX9tCQr.js"
  },
  "/_nuxt/Ckfbl0Rn.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"1cf-AsU/NSIjKO+H0pFS1dcMHC4GMCM\"",
    "mtime": "2026-01-19T22:39:05.077Z",
    "size": 463,
    "path": "../public/_nuxt/Ckfbl0Rn.js"
  },
  "/_nuxt/CKP_9-9R.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"58b9-QbUA+YTruI0OUDicGcAe8fG6LJs\"",
    "mtime": "2026-01-19T22:39:05.074Z",
    "size": 22713,
    "path": "../public/_nuxt/CKP_9-9R.js"
  },
  "/_nuxt/ClXh8knL.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"1d6-BYDAGSGIr0rGWogODOXKW1tfGuM\"",
    "mtime": "2026-01-19T22:39:05.074Z",
    "size": 470,
    "path": "../public/_nuxt/ClXh8knL.js"
  },
  "/_nuxt/Cmz2lAZ1.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"1ae-BLTx7O+hlspTsA2tV0d8vC/eWxg\"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 430,
    "path": "../public/_nuxt/Cmz2lAZ1.js"
  },
  "/_nuxt/Cn3bisnh.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"87-Zi8gMP0kJSzGXH5U57V7pI+xbwg\"",
    "mtime": "2026-01-19T22:39:05.073Z",
    "size": 135,
    "path": "../public/_nuxt/Cn3bisnh.js"
  },
  "/_nuxt/CNhoFLiV.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"be0-dNgH85ufvMKoDBQfnFKCv3Np1V8\"",
    "mtime": "2026-01-19T22:39:05.074Z",
    "size": 3040,
    "path": "../public/_nuxt/CNhoFLiV.js"
  },
  "/_nuxt/CNviGCI-.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"a8-ATtthyd6N5uUc6qGDqp9ik/rdBE\"",
    "mtime": "2026-01-19T22:39:05.073Z",
    "size": 168,
    "path": "../public/_nuxt/CNviGCI-.js"
  },
  "/_nuxt/CnwOumKi.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"6802e-ue0vPyeHom8OhLW7BKvWyAw4a30\"",
    "mtime": "2026-01-19T22:39:05.073Z",
    "size": 426030,
    "path": "../public/_nuxt/CnwOumKi.js"
  },
  "/_nuxt/CNZsU7Hk.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"9ca-vpyN/heZtShncwZwfoTdzmprM4Q\"",
    "mtime": "2026-01-19T22:39:05.077Z",
    "size": 2506,
    "path": "../public/_nuxt/CNZsU7Hk.js"
  },
  "/_nuxt/collaborators.BWsib0yD.css": {
    "type": "text/css; charset=utf-8",
    "etag": "\"1b7e4-O+ZMzT9faE/35NrLtNkNovc7LKg\"",
    "mtime": "2026-01-19T22:39:05.073Z",
    "size": 112612,
    "path": "../public/_nuxt/collaborators.BWsib0yD.css"
  },
  "/_nuxt/CoPT6CbF.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"7df-sdcZ6ld3MD4lifPFQHAe+Bpdb6U\"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 2015,
    "path": "../public/_nuxt/CoPT6CbF.js"
  },
  "/_nuxt/COSxNn6l.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"1f4-8uveq/lcckvYKGvNGHT1cWLTI+Y\"",
    "mtime": "2026-01-19T22:39:05.073Z",
    "size": 500,
    "path": "../public/_nuxt/COSxNn6l.js"
  },
  "/_nuxt/CPlC4uqT.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"4c6-rsLFXCvm/PfqG0WrC9ScPezmoXU\"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 1222,
    "path": "../public/_nuxt/CPlC4uqT.js"
  },
  "/_nuxt/CPXvdkkX.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"2f6-mGvDRXhb1gOOLiQcfGREmnlOG0I\"",
    "mtime": "2026-01-19T22:39:05.074Z",
    "size":
js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"3254-Rq9//iaepszxH7oZX1xYNJQle/M\"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 12884,
    "path": "../public/_nuxt/DYT3rlwz.js"
  },
  "/_nuxt/DzHo6Ocm.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"3c13-sli7OxZCSKK0ST6RMX/vnf2r5is\"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 15379,
    "path": "../public/_nuxt/DzHo6Ocm.js"
  },
  "/_nuxt/e-tw7pCZ.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"797-UV52TY0LI2wwlSy+XDG3hfdGXRk\"",
    "mtime": "2026-01-19T22:39:05.074Z",
    "size": 1943,
    "path": "../public/_nuxt/e-tw7pCZ.js"
  },
  "/_nuxt/EfcOEBmk.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"6b4b-hjZT5wN3Rsbmn7g4sU7kYiVNb/U\"",
    "mtime": "2026-01-19T22:39:05.074Z",
    "size": 27467,
    "path": "../public/_nuxt/EfcOEBmk.js"
  },
  "/_nuxt/entry.ncC07VdV.css": {
    "type": "text/css; charset=utf-8",
    "etag": "\"d611-TjelTbnPlearlG3AStbFQbD9NkE\"",
    "mtime": "2026-01-19T22:39:05.073Z",
    "size": 54801,
    "path": "../public/_nuxt/entry.ncC07VdV.css"
  },
  ...
  "/_nuxt/FileUpload.BEQWAUc7.css": {
    "type": "text/css; charset=utf-8",
    "etag": "\"4eaa-nZ4kKvFdI9gaogLQ2jsArvG9jH0\"",
    "mtime": "2026-01-19T22:39:05.073Z",
    "size": 20138,
    "path": "../public/_nuxt/FileUpload.BEQWAUc7.css"
  },
  "/_nuxt/forgot-password.DwvEOV3N.css": {
    "type": "text/css; charset=utf-8",
    "etag": "\"1e8-3Hde9kuhgsE1N/sZALeSEmit9Uk\"",
    "mtime": "2026-01-19T22:39:05.073Z",
    "size": 488,
    "path": "../public/_nuxt/forgot-password.DwvEOV3N.css"
  },
  "/_nuxt/H0yR8wm4.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"f3-NzBK5t2YGFdlBrh8s9Fd0zpMXIM\"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 243,
    "path": "../public/_nuxt/H0yR8wm4.js"
  },
  "/_nuxt/ify9xXtU.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"f1-QnDzgP6ARcbgTA4cUfqyW56Zals\"",
    "mtime": "2026-01-19T22:39:05.077Z",
    "size": 241,
    "path": "../public/_nuxt/ify9xXtU.js"
  },
  "/_nuxt/IvNa-LGI.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"12f-SOsX571iPbacRLiVfdgchfNdwcw\"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 303,
    "path": "../public/_nuxt/IvNa-LGI.js"
  },
  "/_nuxt/jCl_hRw_.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"4d9-n9u1/CcAuCLGyOK6QVEYg0mDYcQ\"",
    "mtime": "2026-01-19T22:39:05.073Z",
    "size": 1241,
    "path": "../public/_nuxt/jCl_hRw_.js"
  },
  "/_nuxt/JUvUHQVS.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"2c07-Zvko2msS0VqaDu8jiz7TVEHS/+Q\"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 11271,
    "path": "../public/_nuxt/JUvUHQVS.js"
  },
  "/_nuxt/kwsG5Chf.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"1f5b-EBddYfHKr1QtuUpyT/GCOE7lKio\"",
    "mtime": "2026-01-19T22:39:05.077Z",
    "size": 8027,
    "path": "../public/_nuxt/kwsG5Chf.js"
  },
  "/_nuxt/LQNpooBV.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"2c5b-PHikTW2f/hJUhq8OxCGdLC31pfs\"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 11355,
    "path": "../public/_nuxt/LQNpooBV.js"
  },
  "/_nuxt/m0UU3fWF.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"e67-aB8JTRA17LcPuBfA2Hm813TV57E\"",
    "mtime": "2026-01-19T22:39:05.077Z",
    "size": 3687,
    "path": "../public/_nuxt/m0UU3fWF.js"
  },
  "/_nuxt/MKxBbWxK.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"b562-09rMzfVwNb83/6syPAK1OstBwzY\"",
    "mtime": "2026-01-19T22:39:05.074Z",
    "size": 46434,
    "path": "../public/_nuxt/MKxBbWxK.js"
  },
  ...
  "/_nuxt/Montserrat-ExtraLight.CaaIJGL_.ttf": {
    ...
    "mtime": "2026-01-19T22:39:05.072Z",
    "size": 330488,
  ...
  "/_nuxt/NeRbdnaH.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"5615-D+MQ+M4u9a2iC7Qw6YuIucNzPr8\"",
    "mtime": "2026-01-19T22:39:05.077Z",
    "size": 22037,
    "path": "../public/_nuxt/NeRbdnaH.js"
  },
  "/_nuxt/oJAUxEPy.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"120a-ALMwXLZhAz77smJCneKrZD4JGXA\"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 4618,
    "path": "../public/_nuxt/oJAUxEPy.js"
  },
  "/_nuxt/ooU7nbUP.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"1185-gCxyuH5TQ39/48V+FgDW1GV32UI\"",
    "mtime": "2026-01-19T22:39:05.074Z",
    "size": 4485,
    "path": "../public/_nuxt/ooU7nbUP.js"
  },
  "/_nuxt/Outline.BGN8Ra7b.css": {
    "type": "text/css; charset=utf-8",
    "etag": "\"332-vL3CWsCpmvkA24y+XPLumiesAik\"",
    "mtime": "2026-01-19T22:39:05.073Z",
    "size": 818,
    "path": "../public/_nuxt/Outline.BGN8Ra7b.css"
  },
  "/_nuxt/pHdz9RWe.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"a43d-V59zEysjKRCCi9BKjp7GIzavLKY\"",
    "mtime": "2026-01-19T22:39:05.074Z",
    "size": 42045,
    "path": "../public/_nuxt/pHdz9RWe.js"
  },
  "/_nuxt/Primary.C2Wu4kju.css": {
    "type": "text/css; charset=utf-8",
    "etag": "\"113-Nwd60+N09eqnQ2OzPROs/Yi/BMs\"",
    "mtime": "2026-01-19T22:39:05.073Z",
    "size": 275,
    "path": "../public/_nuxt/Primary.C2Wu4kju.css"
  },
  ...
  "/_nuxt/primeicons.DMOk5skT.eot": {
    ...
    "mtime": "2026-01-19T22:39:05.050Z",
    "size": 85156,
  ...
  "/_nuxt/pROLts8n.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"df-1iGGR0+Lwy1OOggYTx2MoVIhO9w\"",
    "mtime": "2026-01-19T22:39:05.073Z",
    "size": 223,
    "path": "../public/_nuxt/pROLts8n.js"
  },
  "/_nuxt/r1p5_YKU.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"a6d2-0yu7Vo449jvaEFArh0o1ODu7KYs\"",
    "mtime": "2026-01-19T22:39:05.075Z",
    "size": 42706,
    "path": "../public/_nuxt/r1p5_YKU.js"
  },
  "/_nuxt/ruDsrGkv.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"95e-pknITMzAGWg5upo8qsx8wcz73vg\"",
    "mtime": "2026-01-19T22:39:05.074Z",
    "size": 2398,
    "path": "../public/_nuxt/ruDsrGkv.js"
  },
  "/_nuxt/s19aRkAY.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"d2c-+EgC+RW/rzxtpz5bNxF2RjajZH4\"",
    "mtime": "2026-01-19T22:39:05.077Z",
    "size": 3372,
    "path": "../public/_nuxt/s19aRkAY.js"
  },
  "/_nuxt/Select.DjffVUJg.css": {
    "type": "text/css; charset=utf-8",
    "etag": "\"19d-yRY6jOWqFsqZAK61VntRZ/D1TTg\"",
    "mtime": "2026-01-19T22:39:05.073Z",
    "size": 413,
    "path": "../public/_nuxt/Select.DjffVUJg.css"
  },
  "/_nuxt/signin.CGGbiHHP.css": {
    "type": "text/css; charset=utf-8",
    "etag": "\"165-tUSqWizEREU2q5qa7uYxSI3NEY4\"",
    "mtime": "2026-01-19T22:39:05.073Z",
    "size": 357,
    "path": "../public/_nuxt/signin.CGGbiHHP.css"
  },
  "/_nuxt/SJuyvVVb.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"1b4-D5EyegIzBfSWpPPjZK8O472adj4\"",
    "mtime": "2026-01-19T22:39:05.074Z",
    "size": 436,
    "path": "../public/_nuxt/SJuyvVVb.js"
  },
  "/_nuxt/t4Il5WOe.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"9c9-brUfuNxyGVUDyuaSrxxw5b80a54\"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 2505,
    "path": "../public/_nuxt/t4Il5WOe.js"
  },
  "/_nuxt/TdniOXf8.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"744-GQyN9u3G66HK6LrQRpADjMvosdg\"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 1860,
    "path": "../public/_nuxt/TdniOXf8.js"
  },
  "/_nuxt/transfer.D00TLzF5.css": {
    "type": "text/css; charset=utf-8",
    "etag": "\"45e-c1rpWyuA3u3bEKJJ+Q+2ZlzpBT8\"",
    "mtime": "2026-01-19T22:39:05.073Z",
    "size": 1118,
    "path": "../public/_nuxt/transfer.D00TLzF5.css"
  },
  "/_nuxt/ucGdFp9s.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"15ca-Engv2zYJXaFRhQDgW2RdSgHZHMY\"",
    "mtime": "2026-01-19T22:39:05.073Z",
    "size": 5578,
    "path": "../public/_nuxt/ucGdFp9s.js"
  },
  "/_nuxt/Uplmo6ZT.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"e1-OLa4Q42ebxYjSjptwJh6hJ3K6UQ\"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 225,
    "path": "../public/_nuxt/Uplmo6ZT.js"
  },
  "/_nuxt/V5gvfDLZ.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"218-yoFBioj1APfKPqlpU4CUm4iDDaM\"",
    "mtime": "2026-01-19T22:39:05.074Z",
    "size": 536,
    "path": "../public/_nuxt/V5gvfDLZ.js"
  },
  "/_nuxt/V6CfoPFm.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"3502-jUtRJmTE5DHSDA9lfXQOMr0ypoI\"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 13570,
    "path": "../public/_nuxt/V6CfoPFm.js"
  },
  "/_nuxt/vAKCZq_C.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"bd0-8X6RU/CQ/tSz/kr+BSsgjyOCdc8\"",
    "mtime": "2026-01-19T22:39:05.074Z",
    "size": 3024,
    "path": "../public/_nuxt/vAKCZq_C.js"
  },
  "/_nuxt/WL9ePIg8.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"35ad-SghASzIY3s/q90PlXlO/XCxZNYI\"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 13741,
    "path": "../public/_nuxt/WL9ePIg8.js"
  },
  "/_nuxt/WsxhSSXT.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"6c2-M7rnoEhbQBS/AMMarVCDSRUwUv8\"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 1730,
    "path": "../public/_nuxt/WsxhSSXT.js"
  },
  "/_nuxt/XKJPsqUM.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"1594-svTgyiqsZ7+Fi1xj+bnzD6tl3ZY\"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 5524,
    "path": "../public/_nuxt/XKJPsqUM.js"
  },
  "/_nuxt/Xr9JnRGK.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"2216-I12MrFcBC2drVSOYGCU5ZRQ9N/I\"",
    "mtime": "2026-01-19T22:39:05.077Z",
    "size": 8726,
    "path": "../public/_nuxt/Xr9JnRGK.js"
  },
  "/_nuxt/xWU76YDq.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"f3a-KTX06quG2K1Pl2wc00qD7w6ZmJU\"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 3898,
    "path": "../public/_nuxt/xWU76YDq.js"
  },
  "/_nuxt/zHnSOxTp.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"6c1-DjiCldRFOgz8OLTXnwS1gxpw+eE\"",
    "mtime": "2026-01-19T22:39:05.076Z",
    "size": 1729,
    "path": "../public/_nuxt/zHnSOxTp.js"
  },
  "/_nuxt/_2OAnPoV.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"18215-MTwnZ52ffZtQNrAki6g7Bp8jh3s\"",
    "mtime": "2026-01-19T22:39:05.074Z",
    "size": 98837,
    "path": "../public/_nuxt/_2OAnPoV.js"
  },
  "/_nuxt/_SCe7BDU.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"ef-w0TGGZyZa9QpjJm60450k3CGX48\"",
    "mtime": "2026-01-19T22:39:05.073Z",
    "size": 239,
    "path": "../public/_nuxt/_SCe7BDU.js"
  },
  "/_nuxt/builds/latest.json": {
    "type": "application/json",
    "etag": "\"47-22O3jSJngA2NnMPGogBcQmpuc60\"",
    "mtime": "2026-01-19T22:39:32.434Z",
    "size": 71,
    "path": "../public/_nuxt/builds/latest.json"
  },
  "/_nuxt/builds/meta/89f8dc1d-2324-45f4-9b06-218022ee3f26.json": {
    "type": "application/json",
    "etag": "\"8b-AjkIHkYRRZubKcT052XrQFQ1rNA\"",
    "mtime": "2026-01-19T22:39:32.435Z",
    "size": 139,
    "path": "../public/_nuxt/builds/meta/89f8dc1d-2324-45f4-9b06-218022ee3f26.json"
  }
};
...
const _lazy_2EibE7 = () => import('../routes/api/auth/validateOtp.mjs');
...
const _lazy_zDr8Xz = () => import('../routes/api/demand/requestOtp.post.mjs');
const _lazy_eJmVG8 = () => import('../routes/api/demand/search.post.mjs');
const _lazy_uNUzV8 = () => import('../routes/api/demand/setting.post.mjs');
const _lazy_c_ayeD = () => import('../routes/api/demand/types.post.mjs');
const _lazy_A5ybgz = () => import('../routes/api/demand/validateOtp.post.mjs');
const _lazy_MJ_8ij = () => import('../routes/api/profile/accounts.get.mjs');
const _lazy_l9aoER = () => import('../routes/api/profile/collaboratorSearch.mjs');
const _lazy_zuhYYs = () => import('../routes/api/profile/collaboratorsList.post.mjs');
const _lazy_IrEJAn = () => import('../routes/api/profile/manager.post.mjs');
const _lazy_cDN2GO = () => import('../routes/api/seed/currency.post.mjs');
...
const handlers = [
  ...
  { route: '/api/auth/validateOtp', handler: _lazy_2EibE7, lazy: true, middleware: false, method: undefined },
  ...
  { route: '/api/demand/requestOtp', handler: _lazy_zDr8Xz, lazy: true, middleware: false, method: "post" },
  { route: '/api/demand/search', handler: _lazy_eJmVG8, lazy: true, middleware: false, method: "post" },
  { route: '/api/demand/setting', handler: _lazy_uNUzV8, lazy: true, middleware: false, method: "post" },
  { route: '/api/demand/types', handler: _lazy_c_ayeD, lazy: true, middleware: false, method: "post" },
  { route: '/api/demand/validateOtp', handler: _lazy_A5ybgz, lazy: true, middleware: false, method: "post" },
  { route: '/api/profile/accounts', handler: _lazy_MJ_8ij, lazy: true, middleware: false, method: "get" },
  { route: '/api/profile/collaboratorSearch', handler: _lazy_l9aoER, lazy: true, middleware: false, method: undefined },
  { route: '/api/profile/collaboratorsList', handler: _lazy_zuhYYs, lazy: true, middleware: false, method: "post" },
  { route: '/api/profile/manager', handler: _lazy_IrEJAn, lazy: true, middleware: false, method: "post" },
  { route: '/api/seed/currency', handler: _lazy_cDN2GO, lazy: true, middleware: false, method: "post" },
...
export { $fetch$1 as $, searchCollaborators as A, saveCollaborators as B, users as C, createCollaborators as D, saveCurrency as E, hasProtocol as F, isScriptProtocol as G, joinURL as H, withQuery as I, destr as J, klona as K, sanitizeStatusCode as L, parse as M, getRequestHeader as N, isEqual as O, getContext as P, setCookie as Q, getCookie as R, deleteCookie as S, baseURL as T, createHooks as U, executeAsync as V, toRouteMatcher as W, createRouter$1 as X, defu as Y, getRequestURL as Z, createDefu as _, readValidatedBody as a, withoutTrailingSlash as a0, parsePath as a1, parseQuery as a2, withTrailingSlash as a3, buildAssetsURL as a4, appRootTag as a5, appRootAttrs as a6, appSpaLoaderTag as a7, appSpaLoaderAttrs as a8, getResponseStatusText as a9, getResponseStatus as aa, appId as ab, defineRenderHandler as ac, publicAssetsURL as ad, appTeleportTag as ae, appTeleportAttrs as af, getQuery as ag, appHead as ah, getRouteRules as ai, useNitroApp as aj, toNodeListener as ak, trapUnhandledNodeErrors as al, setupGracefulShutdown as am, authCookie as b, createError$1 as c, defineEventHandler as d, errorMap as e, fetch as f, deleteUser as g, getOtp as h, saveBeneficiary as i, db as j, countries as k, loadLocale as l, saveCountries as m, saveFiles as n, beneficiaryFiles as o, saveTransfer as p, updateTransferOtp as q, readBody as r, saveUser as s, searchTransfers as t, useRuntimeConfig as u, getCurrency as v, getAccounts as w, beneficiaries as x, saveTransferTypes as y, isOtpValidate as z };
