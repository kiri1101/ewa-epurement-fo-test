import { d as defineEventHandler, u as useRuntimeConfig, r as readBody, l as loadLocale, f as fetch, a as readValidatedBody, c as createError, B as saveCollaborators, e as errorMap } from '../../../_/nitro.mjs';
...
const collaboratorsList_post = defineEventHandler(async (event) => {
  ...
  const response = payload.success ? await api(config.private.api.auth.clientUser.list, {
    method: "POST",
    body: {
  ...
  if (response) {
    if (String(response == null ? void 0 : response.pesake.code).length > 0) {
      ...
      output = await saveCollaborators(event, response == null ? void 0 : response.data.users);
    }
  }
  return {
...
export { collaboratorsList_post as default };
//# sourceMappingURL=collaboratorsList.post.mjs.map
