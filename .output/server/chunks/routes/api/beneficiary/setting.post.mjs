import { d as defineEventHandler, u as useRuntimeConfig, r as readBody, l as loadLocale, f as fetch, a as readValidatedBody, j as db, k as countries, c as createError, m as saveCountries, n as saveFiles, o as beneficiaryFiles, e as errorMap } from '../../../_/nitro.mjs';
...
const setting_post = defineEventHandler(async (event) => {
  ...
  const countriesPresentState = await db.select().from(countries).where(eq(countries.createdAt, now));
  if (countriesPresentState.length === 0) {
    const response = payload.success ? await api(config.private.api.beneficiary.setting, {
      method: "POST",
      body: {
    ...
    if (response) {
      if (String(response == null ? void 0 : response.pesake.code).length > 0) {
        ...
        const countryResponse = await saveCountries(
          event,
          response == null ? void 0 : response.data.authorizedCountries,
          now
        );
        const fileResponse = await saveFiles(
          event,
          response == null ? void 0 : response.data.beneficialConfig,
          now
        );
        output = {
    ...
    const filesResponse = await db.select({
    ...
    }).from(beneficiaryFiles).where(eq(beneficiaryFiles.createdAt, now));
    output = {
