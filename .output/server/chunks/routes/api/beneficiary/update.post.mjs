...
const update_post = defineEventHandler(
  async (event) => {
    ...
    const loginSchema = z.object({
      ...
      mailingAddress: reqBody.mailingAddress.trim().length > 0 ? z.email({
        error: () => ({ message: t.email })
      }) : z.string().optional(),
      phone: reqBody.phone.trim().length > 0 ? z.string({
        error: () => ({ message: t.required })
      }) : z.string().optional(),
      countryCode: reqBody.countryCode.trim().length > 0 ? z.string({
        error: () => ({ message: t.required })
      }) : z.string().optional(),
      country: z.string({
        error: () => ({ message: t.required })
    ...
    const response = payload.success ? await api(config.private.api.beneficiary.update, {
      method: "POST",
      body: {
