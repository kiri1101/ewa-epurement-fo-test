...
const currency_post = defineEventHandler(async (event) => {
  ...
  const response = payload.success ? await api(config.private.api.setting.currency, {
    method: "POST",
    body: {
  ...
  if (response) {
    ...
    output = response == null ? void 0 : response.data.map((currency) => {
      return {
        uuid: crypto.randomUUID(),
        value: currency.currencyCode,
        name: currency.currencyLable,
        floatNumber: currency.nombreDecimals
      };
    });
  }
  return {
...
export { currency_post as default };
//# sourceMappingURL=currency.post.mjs.map
