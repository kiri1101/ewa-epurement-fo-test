import { d as defineEventHandler, u as useRuntimeConfig, r as readBody, l as loadLocale, f as fetch, a as readValidatedBody, z as isOtpValidate, c as createError, e as errorMap } from '../../../_/nitro.mjs';
...
const validateOtp_post = defineEventHandler(async (event) => {
  ...
  if (payload.success) {
    console.error("validate OTP payload: ", {
      demandeSlug: payload.data.demandCode,
      otpCode: payload.data.otp,
      uploadedFiles: payload.data.fileCode.length > 0 ? [payload.data.fileCode] : [],
      lang: payload.data.lang.toUpperCase(),
      origin: config.private.origin.toUpperCase()
    });
    const otpStatus = await isOtpValidate(
      payload.data.demandCode,
      payload.data.otp
    );
    if (!otpStatus) {
      throw createError({
        statusCode: 500,
        statusText: t.invalidOtp
      });
    }
  }
  const response = payload.success ? await api(config.private.api.transfer.otp.validate, {
    method: "POST",
    body: {
      demandeSlug: payload.data.demandCode,
      otpCode: payload.data.otp,
      uploadedFiles: [payload.data.fileCode],
      lang: payload.data.lang.toUpperCase(),
      origin: config.private.origin.toUpperCase()
    }
  }).catch((error) => {
  ...
  if (response) {
    console.error("validate OTP response: ", response);
...
export { validateOtp_post as default };
//# sourceMappingURL=validateOtp.post.mjs.map
