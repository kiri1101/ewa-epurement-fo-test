import { d as defineEventHandler, u as useRuntimeConfig, r as readBody, l as loadLocale, f as fetch, a as readValidatedBody, c as createError, p as saveTransfer, e as errorMap } from '../../../_/nitro.mjs';
...
const list_post = defineEventHandler(async (event) => {
  ...
  const response = payload.success ? await api(config.private.api.transfer.list, {
    method: "POST",
    body: {
  ...
  if (response) {
    if (String(response == null ? void 0 : response.pesake.code).length > 0) {
      ...
      output = await saveTransfer(event, response == null ? void 0 : response.data);
    }
  }
  return {
