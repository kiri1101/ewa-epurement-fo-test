...
const engagement_post = defineEventHandler(async (event) => {
  ...
  const response = payload.success ? await api(config.private.api.transfer.engagement, {
    method: "POST",
    body: {
  ...
  if (response) {
    if (String(response == null ? void 0 : response.pesake.code).length > 0) {
      ...
      output = {
        name: response == null ? void 0 : response.data.fileName,
        link: response == null ? void 0 : response.data.downloadLink
      };
    }
  }
  return {
...
export { engagement_post as default };
//# sourceMappingURL=engagement.post.mjs.map
