...
type Response = {
  pesake: Pesake
  data: TransferAttachmentResponse[]
}
...
export default defineEventHandler(async (event: H3Event) => {
  ...
  const response: Response | null = payload.success
    ? ((await api(config.private.api.transfer.demandFile, {
        method: 'POST',
        body: {
          demandeSlug: payload.data.demandId,
  ...
  if (response) {
    if (String(response?.pesake.code).length > 0) {
      ...
      output = response?.data.map((attachment: TransferAttachmentResponse) => {
